// WIP
// Simple example of utility functions

import { _, createGroup, createLinkGroup } from "./utility.js";
document.documentElement.dataset.noLinkOpenButtons = '';
createGroup({ legendText: 'Test Content', content: [_('img', { src: 'data:image/svg+xml;base64,' })] });
createLinkGroup({
    legendText: 'Test Links',
    staticLinks: [
        { innerText: 'Search', href: 'https://duckduckgo.com' },
        {
            imgSrc: 'data:image/svg+xml;base64,',
            href: 'https://duckduckgo.com'
        }
    ],
    inputLinks: [
        {
            imgSrc: 'data:image/svg+xml;base64,',
            href: 'https://duckduckgo.com/?q={{{PLACEHOLDER}}}'
        },
        {
            innerText: 'Search',
            href: 'https://duckduckgo.com/?q={{{PLACEHOLDER}}}'
        }
    ],
});
createGroup({ legendText: 'Test grow', content: [_('div', { style: { height: '200px' } })], defaultShrink: true });
createGroup({ legendText: 'Test shrink', content: [_('div', { style: { height: '200px' } })] });